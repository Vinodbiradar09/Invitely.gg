...
// Each prepared uniform can hold one of these value shapes
type PreparedUniformValue =
  | { value: number; type: string }
  | { value: number[]; type: string }
  | { value: THREE.Vector2; type: string }
  | { value: THREE.Vector2 } // u_resolution (no type key)
...
const ShaderMaterial = ({
  ...
  // Mutable frame-timing value — ref avoids the immutability lint error
  const lastFrameTimeRef = useRef(0);

  const getUniforms = useMemo(() => {
    return (): Record<string, PreparedUniformValue> => {
      const preparedUniforms: Record<string, PreparedUniformValue> = {};
